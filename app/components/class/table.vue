    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <UTable v-if="view === 'table'" class="hidden md:block" :columns="columns" :data="data" :loading="loading">
        ...
        <template #actions-cell="{ row }">
          <UButton @click="viewClass(row.original)" size="sm" variant="ghost" color="success" class="cursor-pointer"
            :icon="VIEW_ICON" />
        </template>
      </UTable>
      ...
      <template v-if="loading">
        <UCard v-for="i in 6" :key="i" variant="outline" class="overflow-hidden" :ui="{ body: 'p-0' }">
          <!-- Header -->
          ...
          <div class="grid grid-cols-2 gap-3 p-5">
            <div v-for="j in 4" :key="j" class="rounded-2xl border border-default bg-gray-100 p-4 dark:bg-neutral-800">
      ...
      <template v-else-if="data?.length">
        <UCard v-for="item in data" :key="item.id" variant="outline"
          ...
          :ui="{ body: 'p-0' }">
          <!-- Header -->
          <div class="relative overflow-hidden border-b border-default p-3 md:p-0 md:pb-3">
            <div class="flex items-start justify-between ">
            ...
            <div
              ...
              <div class="mb-3 flex items-center gap-2">
                <div class="flex size-7 items-center justify-center rounded-lg bg-amber-100 dark:bg-amber-500/20">
                  <UIcon name="i-lucide-layers-3" class="size-4 text-amber-600 dark:text-amber-400" />
                </div>

                <span class="text-[11px] font-medium uppercase tracking-wide text-amber-700 dark:text-amber-300">
                  Level
                </span>
              </div>
            ...
            <div
              ...
              <div class="mb-3 flex items-center gap-2">
                <div class="flex size-7 items-center justify-center rounded-lg bg-violet-100 dark:bg-violet-500/20">
                  <UIcon name="i-lucide-users" class="size-4 text-violet-600 dark:text-violet-400" />
                </div>
          ...
          <div class="flex items-center justify-between border-t border-default p-3 md:p-0 md:pt-3">
            ...
            <UButton icon="i-lucide-arrow-right" color="neutral" variant="soft" square
              class="rounded-xl transition-all group-hover:translate-x-1" @click="viewClass(item)" />
          </div>
        </UCard>
      </template>
