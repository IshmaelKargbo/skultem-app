...
const recipientsPreview = computed(() => audienceSize[state.audience])
...
async function onSubmit() {
  ...
  try {
    const broadcast = store.composeBroadcast({
      title: state.title,
...
<template>
  ...
  <!-- Composer -->
  <UCard
    class="overflow-hidden border-default/70 shadow-sm"
    :ui="{ body: 'p-0' }"
  >
    <!-- Header -->
      ...
      >
        ...
        >
          <UInput
            ...
            placeholder="e.g. Term 2 Resumption Notice"
            :disabled="isLoading"
            class="w-full"
            size="lg"
          />
        ...
        >
          ...
          <template #hint>
            <span
              class="text-xs"
              :class="state.message.length > 450
                ? 'text-warning'
                : 'text-muted'"
            >
        ...
        >
          <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <button
              v-for="channel in broadcastChannels"
              :key="channel.value"
              type="button"
              :disabled="isLoading"
              ...
              :class="
                state.channels.includes(channel.value as BroadcastChannel)
            ...
            >
              ...
              <UIcon
                :name="channel.icon || 'i-lucide-send'"
        ...
        <!-- Schedule -->
        <UFormField
          v-if="state.sendOption === 'SCHEDULE'"
          label="Schedule For"
          name="scheduledAt"
          required
        >
    ...
    >
      <div class="">
        <div class="flex items-start justify-between gap-4">
          ...
          <div
            class="flex size-12 items-center justify-center rounded-2xl bg-primary/10"
          >
            <UIcon
              name="i-lucide-users"
        ...
        <div class="mt-5 rounded-xl bg-gray-100 dark:bg-gray-800 px-3 py-2.5">
    ...
    <UCard>
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <UBadge
            :color="state.sendOption === 'NOW' ? 'success' : 'primary'"
            variant="soft"
          >
        ...
        <div
          v-if="state.sendOption === 'SCHEDULE' && state.scheduledAt"
    ...
    <UCard v-if="lastSent">
      <template #header>
        <div class="flex items-center justify-between gap-3">
          ...
          <UBadge
            :color="broadcastStatusStyle[lastSent.status].color"
            variant="soft"
            size="xs"
          >
      ...
      <div>
        ...
        <UButton
          to="/communicate/broadcast/history"
          label="View History"
          variant="link"
          size="sm"
          class="mt-3 px-0"
