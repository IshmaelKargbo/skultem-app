<template>
  <div class="p-4 md:px-6 space-y-4">
    ...
    >
      <div v-if="hasDraftAssessments" class="grid w-full gap-2 grid-cols-2 md:w-auto md:flex-wrap md:justify-end">
        ...
        <UButton
          ...
          :loading="completing"
          variant="subtle"
          :disabled="disableActions || !hasDraftAssessments"
    ...
    <UCard class="hidden md:block">
      <div class="flex space-x-3">
        ...
        <USelectMenu
          ...
          v-model="state.teacherSubjectId"
          @change="fetchStudents"
        />
      </div>
      <template v-if="assessments.length && state.teacherSubjectId" #footer>
        <div class="space-y-3">
          <div class="flex gap-3 md:flex-row items-center justify-between mb-3">
            ...
            <UBadge
              :label="`Progress ${workflowProgress}%`"
              :color="workflowProgress === 100 ? 'success' : 'warning'"
              variant="outline"
        ...
        <template #student-cell="{ row }">
          <StudentIdentityCell
            ...
            :photo="
              ...
              row.original.student?.photo
            "
            :subtitle="
              hasSubmittedAssessments
                ? `Position: ${rankingMap[row.original.id] || '-'}`
                : 'Position: N/A'
            "
          />
        </template>
    ...
    <div
      v-if="state.teacherSubjectId && rows.length"
      class="grid gap-4"
      :class="
        view === 'table' ? 'md:hidden' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3'
      "
    >
      <UCard
      ...
      >
        ...
        <div class="divide-y divide-gray-100 dark:divide-white/5">
          ...
          >
            ...
            <div
              v-if="isEditableStatus(getStudentScore(student, assessment.id)?.status as ScoreStatus)"
              class="w-24 shrink-0"
            >
              <UInput
                size="md"
                ...
                :model-value="getStudentScore(student, assessment.id)?.score"
                type="number"
                min="0"
                max="100"
            ...
            <div v-else class="flex flex-col items-end gap-1 shrink-0">
              <UBadge
                size="sm"
                variant="soft"
                :color="statusBadgeColor(assessment.status)"
              >
    ...
    <div class="grid gap-3 grid-cols-2 lg:grid-cols-5">
      <Metric
        ...
        v-for="(summary, i) in statusSummary"
        :key="summary.status"
        :record="{
          color: summary.color,
          icon: summary.icon,
...
const classes = computed(() =>
  classSessionStore.records.map((e) => ({ label: parseClassSession(e), value: e.id }))
);
...
function calculateGrade(total: number, row: any) {
  ...
  const band = gradingBands.value.find(
    (item) => total >= item.minScore && total <= item.maxScore
  );
  return band?.grade || "-";
}
...
async function fetchRecord() {
  ...
  try {
    ...
    teachers.value = (res || []).map((e: TeacherSubject) => ({
      label: `${e.subjectName} (${e.teacherName})`,
      value: e.id,
    }));
...
function buildColumns() {
  ...
  const assessmentColumns = assessments.value.map((a) => ({
    ...
    cell: ({ row }: any) => {
      const idx = row.original.scores.findIndex((s: any) => s.assessment === a.id);

      if (idx < 0) return h("div", { class: "text-gray-400" }, "-");
  ...
  const summaryColumns = [
    ...
    },
    {
      ...
      footer: () =>
        hasSubmittedAssessments.value ? "Ranked" : "Ranking pending submission",
    },
    {
      id: "grade",
      header: "Grade",
...
function buildAssessmentGrades(assessmentId: string) {
  return rows.value
    .map((student) => student.scores.find((s) => s.assessment === assessmentId))
...
async function persistDraftGrades(showToast: boolean) {
  ...
  try {
    await Promise.all(
      editable.map((a) =>
        store.grade(state.teacherSubjectId, {
          assessmentId: a.id,
          termId: state.termId,
          grades: buildAssessmentGrades(a.id),
        })
      )
    );
    await fetchStudents();
...
async function completeAssessment() {
  ...
  try {
    ...
    await Promise.all(
      editable.map((a) =>
        store.submit(state.teacherSubjectId, {
          assessmentId: a.id,
          termId: state.termId,
          note: "Submitted from grade entry",
        })
      )
    );
    await fetchStudents();
