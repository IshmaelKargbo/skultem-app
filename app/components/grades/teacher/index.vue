...
const term = computed(() => termStore.records.find(e => e.status == 'ACTIVE'))
...
const continuousAssessments = computed(() => assessments.value.flatMap(a => {
...
async function fetchRecord() {
  ...
  try {
    ...
    const res = await useTeacherSubjectStore().fetchAllByTeacher(0, 0)
    records.value = res
    state.teacherSubjectId = subjects.value[0]?.value || ""
    await fetchStudents()
  } catch (error: any) {
    records.value = []
...
async function persistDraftGrades(showToast: boolean) {
  ...
  if (!editable.length) {
    if (showToast) info(continuousIds.value.size ? 'Continuous assessments are recorded on their own page - use Record CA' : 'No editable assessments available')
    return true
  }
  saving.value = true
  try {
