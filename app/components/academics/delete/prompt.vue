...
async function remove() {
  try {
    loading.value = true
    await store.remove(props.yearId)
    toastSuccess('Academic year deleted successfully')
    close()
  } catch (error: any) {
    toastError(error?.message || 'Failed to delete academic year')
  } finally {
    loading.value = false
  }
}
      ...
      <UCard>
        ...
        <div class="space-y-4">
          <UAlert
            ...
            :description="`Remove '${yearName}'? Only an inactive, non-closed academic year can be deleted.`"
          />
        </div>
