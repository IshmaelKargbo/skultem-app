...
<template>
  ...
  <!-- Mobile -->
  <div class="bg-gray-100 dark:bg-neutral-950 min-h-[300px]" :class="{ 'md:hidden': view === 'table' }">
    <!-- Loading -->
    <div v-if="loading">
      <div class="space-y-4" :class="{
        ...
        <UCard v-for="i in 6" :key="i" class="overflow-hidden rounded-2xl border border-default" :ui="{ body: 'p-0' }">
          <!-- Header -->
    ...
    <!-- Empty -->
    <Ucard v-else-if="!data?.length" class="flex flex-col items-center justify-center px-6 py-16 text-center">
      <div
    ...
    <div class="space-y-4"
      ...
      <UCard v-for="item in data" :key="item.id"
        ...
        <div v-if="!loading && data?.length"
          class="flex flex-col md:flex-row md:justify-between md:w-full items-center gap-3 pt-2 col-span-full"></div>
      </UCard>
      <div v-if="!loading && data?.length"
