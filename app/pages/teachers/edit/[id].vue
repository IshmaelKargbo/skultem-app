      ...
      <UCard>
        ...
        <div class="grid gap-5 md:grid-cols-2">
          ...
          <UFormField label="Email Address" name="email" required>
            <UInput disabled v-model="state.email" placeholder="teacher@example.com" leading-icon="i-lucide-mail" />
            <template #help>
              <p class="text-sm text-muted">
...
const formRef = ref()
const record = ref<Teacher | undefined>()
...
async function onSubmit(event: any) {
  ...
  try {
    ...
    error(
      err?.message || 'Failed to edit teacher'
    )
  } finally {
...
async function fetchRecord() {
  try {
    if (!id) return navigateTo("/teachers")
    ...
    state.givenNames = res.user?.givenNames
    state.familyName = res.user?.familyName
