<template>
  <u-slideover :dismissible="false" title="Create Discount" :open="open" @update:open="open = $event">
    <!-- Trigger -->
    <UButton :color="triggerColor" :label="triggerLabel" :icon="triggerIcon" :variant="triggerVariant"
      :size="triggerSize" @click="open = true" />
    ...
    <template #body>
      <UForm ref="formRef" :schema="schema" :state="state" class="space-y-4 w-full" @submit="onSubmit">
        ...
        <UFormField label="Name" name="name" required>
          <UInput v-model="state.name" icon="i-lucide-tag" placeholder="e.g. Discount for Aruna Kargbo - First Term" />
          <template #help>
            <p class="text-xs text-mute">Enter a descriptive name for the discount.</p>
          </template>
        </UFormField>
        ...
        <UFormField label="Fee Category" name="feeId" required>
          <!-- Fee Category -->
          <USelect v-model="state.feeId" :items="categories" placeholder="Select fee category"
            leading-icon="i-lucide-wallet" @change="changeFee" :disabled="isLoading && !state.studentId" />
          <template #help>
        ...
        <UFormField label="Type" name="kind" required>
          <!-- Discount Type -->
          <USelect v-model="state.kind" :items="kindOptions" placeholder="Select Discount Type"
            leading-icon="i-lucide-badge-percent" @change="kindChange" />
          <template #help>
            <p class="text-xs text-muted">
        ...
        <UFormField label="Value" name="value" required>
          ...
          <UInput v-model="state.value" type="number"
            :icon="state.kind === 'PERCENTAGE' ? 'i-lucide-percent' : 'i-lucide-banknote'"
            placeholder="Enter discount value" :disabled="isLoading" @input="handleAmountInput" />
          <template #help>
            <p class="text-xs text-muted">
              Enter the discount amount or percentage. For percentage, enter a number
              between 1 and 100.
            </p>
          </template>
        </UFormField>

        <div v-if="isRemaning()" class="border border-dashed bg-red-50/50 spacey-1 border-red-300 p-3 rounded-md">
        ...
        <UFormField label="Expiry Date" name="expiryDate" required>
          <!-- Expiry -->
          <UInput v-model="state.expiryDate" type="date" icon="i-lucide-calendar-days" :disabled="isLoading" />

          <template #help>
            <p class="text-xs text-muted">
              The date by which this discount must be applied. After this date, the
        ...
        <UFormField label="Reason" name="reason" required>
          <!-- Reason -->
          <UInput v-model="state.reason" icon="i-lucide-file-text" placeholder="Example: Scholarship award"
            :disabled="isLoading" />
          <template #help>
            <p class="text-xs text-muted">Enter reason for this discount.</p>
          </template>
        </UFormField>
      </UForm>
    </template>
...
async function fetchRecords() {
  try {
    ...
    const result = await useStudentStore().getAllStudentOutstandingFeesById(
      state.studentId
    );
