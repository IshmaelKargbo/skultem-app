<template>
  <div class="p-5 space-y-5">
      ...
      <div class="space-y-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
          ...
          <UBadge :label="`Progress ${workflowProgress}%`" :color="workflowProgress === 100 ? 'success' : 'warning'"
            variant="soft" icon="mdi:chart-timeline-variant" />
        </div>
        <UProgress :color="workflowProgress === 100 ? 'success' : 'warning'" v-model="workflowProgress" />

        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <UCard v-for="summary in statusSummary" :key="summary.status">
    ...
    <UCard v-if="state.teacherSubjectId && rows.length > 0">
...
async function fetchStudents() {
  ...
  try {
    ...
    assessments.value = []
    columns.value = []
    toast.add({ description: error?.data?.message || error?.message || 'Failed to load grading data', color: 'error' })
  } finally {
    loading.value = false
  }
}
  ...
  const assessmentColumns = assessments.value.map(a => ({
    ...
    cell: ({ row }: any) => {
      ...
      if (!isEditableStatus(status))
        return h("div", { class: "text-gray-500 font-medium" }, score.score)
  ...
  const summaryColumns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return calculateGrade(total, row)
      },
...
async function persistDraftGrades(showToast: boolean) {
  ...
  if (!editable.length) { toast.add({ description: 'No editable assessments available', color: 'info' }); return }
  saving.value = true
  try {
    ...
    await fetchStudents()
    if (showToast) toast.add({ description: 'Grades saved successfully', color: 'success' })
    return true
  } catch (error: any) {
    toast.add({ description: error?.data?.message || error?.message || 'Failed to save grades', color: 'error' })
    return false
...
async function completeAssessment() {
  ...
  try {
    ...
    await fetchStudents()
    toast.add({ description: 'Assessment submitted for approval', color: 'success' })
  } catch (error: any) {
    toast.add({ description: error?.data?.message || error?.message || 'Failed to submit assessments', color: 'error' })
